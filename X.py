import streamlit as st import pandas as pd from io import StringIO
...
with st.sidebar: st.header("Parâmetros") faturacao = st.number_input("Faturação bruta mensal (€)", min_value=0.0, value=3000.0, step=100.0, format="%.2f") comissao_pct = st.slider("Comissão plataforma (%)", min_value=0.0, max_value=60.0, value=25.0, step=1.0) combustivel = st.number_input("Combustível mensal (€)", min_value=0.0, value=450.0, step=10.0, format="%.2f") manutencao = st.number_input("Manutenção mensal (€)", min_value=0.0, value=75.0, step=5.0, format="%.2f") seguro = st.number_input("Seguro mensal (€)", min_value=0.0, value=50.0, step=5.0, format="%.2f") outros = st.number_input("Outros custos operacionais (€)", min_value=0.0, value=0.0, step=5.0, format="%.2f")
...
Evita valores negativos para encargos (não há imposto sobre prejuízo neste cálculo simplificado)
...
Mostrar resultados conforme período
